...
public:
	...
	{
		ChatHandler(Player->GetSession()).PSendSysMessage("欢迎来到魔兽世界.");
	}
};
				...
				if (!playerList.isEmpty())
				{
					for (Map::PlayerList::const_iterator playerIteration = playerList.begin(); playerIteration != playerList.end(); ++playerIteration)
					{
						if (Player* playerHandle = playerIteration->GetSource())
						{
							...
							chatHandle.PSendSysMessage("|cffFF0000 [弹性副本]|r|cffFF8000 %s 进入副本 %s. 当前进度玩家人数共 %u位  |r", player->GetName().c_str(), map->GetMapName(), map->GetPlayersCountExceptGMs() + PlayerCountDifficultyOffset);
						}
					}
				}
				//ChatHandler(player).PSendSysMessage("|cffFF0000 [弹性副本]|r|cffFF8000 %s 进入副本 %s. 目前玩家人数 %u (副本难度: %s) |r", player->GetName().c_str(), map->GetMapName(), map->GetPlayersCountExceptGMs() + PlayerCountDifficultyOffset, sObjectMgr->GetRaidBuff(sObjectMgr->isBossCoreA(player->GetMap()->GetInstanceId())));
			}
		}
	}
				...
				if ((map->GetEntry()->IsDungeon()) && !player->IsGameMaster())
				{
					...
					if (!playerList.isEmpty())
					{
						for (Map::PlayerList::const_iterator playerIteration = playerList.begin(); playerIteration != playerList.end(); ++playerIteration)
						{
							...
							{
								ChatHandler chatHandle = ChatHandler(playerHandle->GetSession());
								chatHandle.PSendSysMessage("|cffFF0000 [弹性副本]|r|cffFF8000 %s 离开副本 %s. 当前进度玩家人数共 %u位  |r", player->GetName().c_str(), map->GetMapName(), map->GetPlayersCountExceptGMs() + PlayerCountDifficultyOffset);
							}
						}
					}
				}
...
public:
	...
	void Creature_SelectLevel(const CreatureTemplate *creatureTemplate, Creature* creature)
	{
		...
		if (creature->GetMap()->IsDungeon() || sConfigMgr->GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1)
		{
			...
			//sLog->outString("生物状态更新  (1)");
		}
	}

	void OnAllCreatureUpdate(Creature* creature, uint32 diff)
	{
		//if (PlayerCount == CreatureInfo[creature->GetGUID()].instancePlayerCount)
           //  PlayerCount = creature->GetMap()->GetPlayersCountExceptGMs() + PlayerCountDifficultyOffset;
		if (!(CreatureInfo[creature->GetGUID()].instancePlayerCount == PlayerCount))
		{		
				if (creature->GetMap()->IsDungeon() || creature->GetMap()->IsBattleground() || sConfigMgr->GetIntDefault("VASAutoBalance.DungeonsOnly", 1) < 1)
					ModifyCreatureAttributes(creature);
				CreatureInfo[creature->GetGUID()].instancePlayerCount = PlayerCount;

		}
		//else if (CreatureInfo[creature->GetGUID()].instancePlayerCount == 1)
		//{
		//	sLog->outString("生物状态更新  (%u)");
		//	CreatureInfo[creature->GetGUID()].instancePlayerCount = 2;
		//}
	...
	void ModifyCreatureAttributes(Creature* creature)
	{
		...
		case 40:
			...
			break;
		case 25:
			if (instancePlayerCount <= 5)
			    healthMultiplier = healthMultiplier;
			else
				healthMultiplier = healthMultiplier + (float)((float)(instancePlayerCount - 5) / 10.0f);
			break;
		case 10:
		...
		default:
			healthMultiplier = healthMultiplier / 10.0f;    // default to a 5 man group
		}
		...
		//healthMultiplier = healthMultiplier + 1.0f;
		...
		if (maxNumberOfPlayers == 0) {
			...
			scaledMana = baseMana * healthMultiplier;
			//scaledMana = ((baseMana / maxNumberOfPlayers) * instancePlayerCount);
			// Now Adjusting Damage, this too is linear for now .... this will have to change I suspect.
			//damageMultiplier = ((float)instancePlayerCount / (float)maxNumberOfPlayers) + 1.0f;
			damageMultiplier = healthMultiplier;
		...
		//if (creatureTemplate->Entry != 37540 && creatureTemplate->Entry != 36948)
		//{
			//creature->CastSpell(creature, sWorld->getIntConfig(RAID_BUFF1), true);
