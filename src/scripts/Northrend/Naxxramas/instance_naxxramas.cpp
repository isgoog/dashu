...
public:
    ...
    struct instance_naxxramas_InstanceMapScript : public InstanceScript
    {
		...
		void OnPlayerEnter(Player* player)
		{
			...
			if (group)
			{
				if (group->IsLeader(player->GetGUID()) && player->IsAlive())
				{
					Difficulty realdiff = player->GetDifficulty(player->GetMapId());
					uint32 destInstId = sInstanceSaveMgr->PlayerGetDestinationInstanceId(player, player->GetMapId(), realdiff);
					sObjectMgr->setisBossCoreB(destInstId, player->m_sender);
					sLog->outString("纳克萨玛斯进度ID  (%u)", destInstId);
					sLog->outString("纳克萨玛斯sender  (%u)", player->m_sender);
					sLog->outString("纳克萨玛斯isboss[destInstId]  (%u)", sObjectMgr->isBossCoreA(player->GetMap()->GetInstanceId()));
				}
			...
			ChatHandler(player).PSendSysMessage("|cffFF0000当前副本难度:|r|cffFF8000%s", sObjectMgr->GetRaidBuff(sObjectMgr->isBossCoreA(player->GetMap()->GetInstanceId())));
        ...
        {
			if (sWorld->getIntConfig(RAID_NAXXBUFF) > 0)
				creature->CastSpell(creature, sWorld->getIntConfig(RAID_NAXXBUFF), true);
			//else if (instance->Is25ManRaid() && sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 1)
			///	creature->CastSpell(creature, sWorld->getIntConfig(RAID_BUFF1), true);
			//else if (instance->Is25ManRaid() && sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 2)
			//	creature->CastSpell(creature, sWorld->getIntConfig(RAID_BUFF2), true);
			//else if (instance->Is25ManRaid() && sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 3)
			//	creature->CastSpell(creature, sWorld->getIntConfig(RAID_BUFF3), true);
			//else if (instance->Is25ManRaid() && sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 4)
			//	creature->CastSpell(creature, sWorld->getIntConfig(RAID_BUFF4), true);
			//else if (instance->Is25ManRaid() && sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 5)
			//	creature->CastSpell(creature, sWorld->getIntConfig(RAID_BUFF5), true);
