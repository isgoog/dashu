...
public:
    ...
    struct instance_ulduar_InstanceMapScript : public InstanceScript
    {
        ...
        void OnPlayerEnter(Player* player)
        {
			...
			if (group)
			{
				if (group->IsLeader(player->GetGUID()) && player->IsAlive())
				{
					...
					sLog->outString("°Â¶Å¶û½ø¶ÈID  (%u)", destInstId);
					sLog->outString("°Â¶Å¶ûsender  (%u)", player->m_sender);
					sLog->outString("°Â¶Å¶ûisboss[destInstId]  (%u)", sObjectMgr->isBossCoreA(player->GetMap()->GetInstanceId()));
				}
			}
			else
			{
			...
			ChatHandler(player).PSendSysMessage("|cffFF0000µ±Ç°¸±±¾ÄÑ¶È:|r|cffFF8000%s", sObjectMgr->GetRaidBuff(sObjectMgr->isBossCoreA(player->GetMap()->GetInstanceId())));
        }
        ...
        {
			if (sWorld->getIntConfig(RAID_ULDBUFF) > 0)
				creature->CastSpell(creature, sWorld->getIntConfig(RAID_ULDBUFF), true);
