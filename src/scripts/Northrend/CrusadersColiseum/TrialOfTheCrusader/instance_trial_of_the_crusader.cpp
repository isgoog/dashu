...
public:
    ...
    struct instance_trial_of_the_crusader_InstanceMapScript : public InstanceScript
    {
        ...
        {
			if (sWorld->getIntConfig(RAID_TOCBUFF) > 0)
				creature->CastSpell(creature, sWorld->getIntConfig(RAID_TOCBUFF), true);
                    ...
                    if( data == DONE )
                    {
                        if( ++Counter >= NPC_ChampionGUIDs.size() )
                        {
                            ...
                            if( Creature* c = instance->GetCreature(NPC_TirionGUID) )
                            {
								...
								/*if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 1)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_1;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 2)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_2;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 3)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_3;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 4)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_4;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 5)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_5;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 6)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_6;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 7)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_7;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 8)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_8;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 9)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_9;
								if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 10)
									cacheEntry = GO_CRUSADERS_CACHE_25_H_10;*/
                ...
                case EVENT_SCENE_502:
                    {
                        if( instance->IsHeroic() )
                        {
                            ...
                            else if( instance->GetSpawnMode() == RAID_DIFFICULTY_25MAN_HEROIC )
							{
								/*if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 1)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_1;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 2)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_2;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 3)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_3;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 4)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_4;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 5)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_5;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 6)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_6;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 7)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_7;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 8)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_8;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 9)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_9;
								}
								else if (sObjectMgr->isBossCoreA(instance->GetInstanceId()) == 10)
								{
									tributeChest = GO_TRIBUTE_CHEST_25H_99_10;
								}
								else
								{*/
									if (AttemptsLeft >= 50)
        ...
        void OnPlayerEnter(Player* plr)
        {
            if( instance->IsHeroic() )
            {
                plr->SendUpdateWorldState(UPDATE_STATE_UI_SHOW, 1);//wqcore 屏幕中间显示数值
                plr->SendUpdateWorldState(UPDATE_STATE_UI_COUNT, AttemptsLeft);
            }
            else
			...
			if (group)
			{
				if (group->IsLeader(plr->GetGUID()) && plr->IsAlive())
				{
					Difficulty realdiff = plr->GetDifficulty(plr->GetMapId());
					uint32 destInstId = sInstanceSaveMgr->PlayerGetDestinationInstanceId(plr, plr->GetMapId(), realdiff);
					sObjectMgr->setisBossCoreB(destInstId, plr->GetTocid());
					//sLog->outString("十字军试炼进度ID  (%u)", destInstId);
					//sLog->outString("十字军试炼sender  (%u)", plr->GetTocid());
					//sLog->outString("十字军试炼isboss[destInstId]  (%u)", sObjectMgr->isBossCoreA(plr->GetMap()->GetInstanceId()));
				}
			}
			else
			{
			...
			ChatHandler(plr).PSendSysMessage("|cffFF0000当前副本难度:|r|cffFF8000%s", sObjectMgr->GetRaidBuff(sObjectMgr->isBossCoreA(plr->GetMap()->GetInstanceId())));
        }
