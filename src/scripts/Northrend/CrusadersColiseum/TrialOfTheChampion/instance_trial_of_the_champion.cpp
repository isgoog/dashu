...
public:
    ...
    struct instance_trial_of_the_champion_InstanceMapScript : public InstanceScript
    {
        ...
        void OnCreatureCreate(Creature* creature)
        {
            ...
            switch( creature->GetEntry() )
            {
                ...
                case VEHICLE_ARGENT_BATTLEWORG:
					...
					if (!players.isEmpty())
						if (Player* pPlayer = players.begin()->GetSource())
							if (pPlayer->m_sender != 1 && pPlayer->m_sender != 2)
							{
		...
		if (plr->m_sender == 1)
		{
			plr->SendUpdateWorldState(6006, 0);
			plr->SendUpdateWorldState(6008, 0);
			plr->SendUpdateWorldState(6010, 0);
			SetData(DATA_BOSHU_ENTRY, plr->GetTTsaodang());
			plr->SendUpdateWorldState(UPDATE_STATE_UI_BOSHU, 1);//wqcore 屏幕中间显示数值
			plr->SendUpdateWorldState(UPDATE_STATE_UI_BOSHU_COUNT, boshu);
			plr->SendUpdateWorldState(UPDATE_STATE_UI_SHIJIAN, 1);
			plr->SendUpdateWorldState(UPDATE_STATE_UI_SHIJIAN_COUNT, shijian);
			plr->SendUpdateWorldState(UPDATE_STATE_UI_JIFEN, 1);
			plr->SendUpdateWorldState(UPDATE_STATE_UI_JIFEN_COUNT, plr->GetTTjifen());
		}
		else if (plr->m_sender == 2)
		{
			plr->SendUpdateWorldState(UPDATE_STATE_UI_BOSHU, 0);
			plr->SendUpdateWorldState(UPDATE_STATE_UI_SHIJIAN, 0);
			plr->SendUpdateWorldState(UPDATE_STATE_UI_JIFEN, 0);
			plr->SendUpdateWorldState(6006, 1);//wqcore 屏幕中间显示数值
			plr->SendUpdateWorldState(6007, cengshu);
			plr->SendUpdateWorldState(6008, 1);
			plr->SendUpdateWorldState(6009, daojishi);
			plr->SendUpdateWorldState(6010, 1);
			plr->SendUpdateWorldState(6011, plr->GetTT2jifen());
		}
		else
		{
			plr->SendUpdateWorldState(UPDATE_STATE_UI_BOSHU, 0);
		...
		InstanceCleanupsuoyao();
		InstanceCleanuptongtian();
           
        }

		void OnPlayerLeave(Player* player)//玩家离开副本
		{
			if (player->IsAlive())
			    player->m_sender = 0;
			events.CancelEvent(EVENT_SUMMON_NPC);
        ...
        uint32 GetData(uint32 uiData) const
        {
            switch( uiData )
            {
				...
				case DATA_SHIJIAN_ENTRY:
					return shijian;
				case DATA_BOSHU_ENTRY:
					return boshu;
				case DATA_PLAYER_BOSHU:
					return playerboshu;
				case DATA_DAOJISHI_ENTRY:
					return daojishi;
				case DATA_CENGSHU_ENTRY:
					return cengshu;
            }
        ...
        void SetData(uint32 uiType, uint32 uiData)
        {
            switch( uiType )
            {
				...
				case DATA_CENGSHU_ENTRY:
					cengshu = uiData;
					break;
				case DATA_SUOYAO_STAT:
					if (uiData == 0)
					{
						events.ScheduleEvent(EVENT_SUMMON_NPC, 1000);
					}
					break;
				case DATA_SUOYAO_END:
					if (uiData == 0)
					{
						events.CancelEvent(EVENT_SUMMON_NPC);
					}
					break;
				case DATA_TONGTIAN_STAT:
					if (uiData == 0)
					{
						events.ScheduleEvent(EVENT_SUMMON_NPC_TONGTIAN, 1000);
					}
					break;
				case DATA_ZHAOHUAN1_STAT:
					if (uiData == 0)
					{
						events.ScheduleEvent(EVENT_SUMMON_NPC_ZHAOHUAN1, 1000);
					}
					break;
				case DATA_ZHAOHUAN2_STAT:
					if (uiData == 0)
					{
						events.ScheduleEvent(EVENT_SUMMON_NPC_ZHAOHUAN2, 1000);
					}
					break;
				case DATA_TONGTIAN_END:
					if (uiData == 0)
					{
						events.CancelEvent(EVENT_SUMMON_NPC_TONGTIAN);
					}
					break;
				case DATA_PLAYER_BOSHU:
					if (uiData == 0)
					{
						playerboshu = playerboshu + 1;
					}
					else
					{
						playerboshu = uiData;
		...
		void DoSummonSuoYao(uint32 BossNumber, uint8 BossOrder)
		{
			uint32 CHAMPION_TO_SUMMON = 0;
			uint32 MINION_TO_SUMMON = 0;
			int32 TEXT_ID = 0;
			...
			{
			case 0:
				CHAMPION_TO_SUMMON = sObjectMgr->GetSuoyao(boshu, 1);
				MINION_TO_SUMMON = sObjectMgr->GetSuoyao(boshu, 2);
				TEXT_ID = TEXT_MOKRA_SKILLCRUSHER;
				break;
			case 1:
				CHAMPION_TO_SUMMON = sObjectMgr->GetSuoyao(boshu, 1);
			...
			Position SpawnPos = SpawnPosition;
			//SpawnPos.Relocate(747.7f, 621.14f, 411.09f);
			...
			if (Creature* pBoss = instance->SummonCreature(CHAMPION_TO_SUMMON, SpawnPos))
			{
				...
				//pBoss->ToCreature()->SetReactState(REACT_PASSIVE);
				pBoss->AI()->SetData(BossOrder, (shortver ? 1 : 0));
				pBoss->GetMotionMaster()->MovePoint(0, 747.7f, 621.14f, 411.09f);
				for (uint8 i = 0; i < 3; ++i)
					if (Creature* pAdd = instance->SummonCreature(MINION_TO_SUMMON, SpawnPos))
					{
						NPC_GrandChampionMinionsGUID[BossOrder][i] = pAdd->GetGUID();
		...
		void DoSummonTongTian(uint32 BossNumber, uint8 BossOrder)
		{
			uint32 CHAMPION_TO_SUMMON = 0;
			uint32 MINION_TO_SUMMON = 0;
			int32 TEXT_ID = 0;
			...
			{
			case 0:
				CHAMPION_TO_SUMMON = sObjectMgr->GetTongtian(cengshu);
				MINION_TO_SUMMON = sObjectMgr->GetTongtian(cengshu);
				TEXT_ID = TEXT_MOKRA_SKILLCRUSHER;
				break;
			case 1:
				CHAMPION_TO_SUMMON = sObjectMgr->GetTongtian(cengshu);
		...
		void DoSummonZhaoHuan1(uint32 BossNumber, uint8 BossOrder)
		{
			uint32 CHAMPION_TO_SUMMON = 98001;
			uint32 MINION_TO_SUMMON = 98107;
			int32 TEXT_ID = 0;

			switch (BossNumber)
			{
			case 0:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_MOKRA_SKILLCRUSHER;
				break;
			case 1:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 5;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_ERESSEA_DAWNSINGER;
				break;
			case 2:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 10;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_RUNOK_WILDMANE;
				break;
			case 3:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 15;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_ZUL_TORE;
				break;
			case 4:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 20;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_DEATHSTALKER_VESCERI;
				break;
			case 5:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 25;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_DEATHSTALKER_VESCERI;
				break;
			case 6:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 30;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_DEATHSTALKER_VESCERI;
				break;
			case 7:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 35;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_DEATHSTALKER_VESCERI;
				break;
			case 8:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 40;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_DEATHSTALKER_VESCERI;
				break;
			case 9:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 45;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_DEATHSTALKER_VESCERI;
				break;
			case 10:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + 49;
				MINION_TO_SUMMON = MINION_TO_SUMMON;
				TEXT_ID = TEXT_DEATHSTALKER_VESCERI;
				break;
			default:
		...
		void DoSummonZhaoHuan2(uint32 BossNumber, uint8 BossOrder)
		{
			uint32 CHAMPION_TO_SUMMON = 98051;
			uint32 MINION_TO_SUMMON = 98111;
			int32 TEXT_ID = 0;

			switch (BossNumber)
			{
			case 0:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + urand(0, 9);
				MINION_TO_SUMMON = 98111;
				TEXT_ID = TEXT_MOKRA_SKILLCRUSHER;
				break;
			case 1:
				CHAMPION_TO_SUMMON = CHAMPION_TO_SUMMON + urand(0, 9);
        ...
        void Update(uint32 diff)
        {
            ...
            switch( events.GetEvent() )
            {
				...
				case EVENT_SUMMON_NPC:
				{
					...
					if (!players.isEmpty())
						if (Player* plr = players.begin()->GetSource())
						{
							...
							plr->SendUpdateWorldState(UPDATE_STATE_UI_SHIJIAN_COUNT, shijian);
							plr->SendUpdateWorldState(UPDATE_STATE_UI_JIFEN_COUNT, plr->GetTTjifen());
							if (plr->GetTTboshu() < playerboshu)
							{
								plr->SetTTboshu(playerboshu);
							}
							if (plr->GetTTsaodang() < playerboshu)
							{
								plr->SetTTsaodang(playerboshu);
							}
				...
				break;
				case EVENT_SUMMON_NPC_TONGTIAN:
				{
					if (cengshu != 100)
					{
						cengshu = cengshu + 1;
						HandleGameObject(GO_MainGateGUID, true);
						DoSummonTongTian(urand(0, 4), 0);
					}

					events.CancelEvent(EVENT_SUMMON_NPC_TONGTIAN);
						events.ScheduleEvent(EVENT_GUANMEN, 3000);
					if (daojishi == 600)
					   events.ScheduleEvent(EVENT_JISHI_TONGTIAN, 1000);

				}
				break;
				case EVENT_SUMMON_NPC_ZHAOHUAN1:
				{
						HandleGameObject(GO_MainGateGUID, true);
						DoSummonZhaoHuan1(urand(0, 10), 0);

					events.CancelEvent(EVENT_SUMMON_NPC_ZHAOHUAN1);
					events.ScheduleEvent(EVENT_GUANMEN, 3000);
				}
				break;
				case EVENT_SUMMON_NPC_ZHAOHUAN2:
				{
					HandleGameObject(GO_MainGateGUID, true);
					DoSummonZhaoHuan2(urand(0, 4), 0);

					events.CancelEvent(EVENT_SUMMON_NPC_ZHAOHUAN2);
					events.ScheduleEvent(EVENT_GUANMEN, 3000);
				}
				break;
				case EVENT_JISHI_TONGTIAN:
				{
					if (daojishi > 0)
					{
						daojishi = daojishi - 1;
					...
					events.CancelEvent(EVENT_JISHI_TONGTIAN);
					if (daojishi != 0)
					{
						events.ScheduleEvent(EVENT_JISHI_TONGTIAN, 1000);
					}
				...
				{
					HandleGameObject(GO_MainGateGUID, false);
					events.CancelEvent(EVENT_GUANMEN);
				}
					break;
            }      
