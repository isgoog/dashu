...
class instance_icecrown_citadel : public InstanceMapScript
{
    public:
        ...
        struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript
        {
            ...
            void OnPlayerEnter(Player* player)//玩家进入副本
            {
				...
				if (group)
				{
					if (group->IsLeader(player->GetGUID()) && player->IsAlive())
					{
						...
						sObjectMgr->setisBossCoreB(destInstId, player->GetIccid());
						//sLog->outString("冰冠堡垒进度ID  (%u)", destInstId);
						//sLog->outString("冰冠堡垒sender  (%u)", player->GetIccid());
						//sLog->outString("冰冠堡垒isboss[destInstId]  (%u)", sObjectMgr->isBossCoreA(player->GetMap()->GetInstanceId()));
					}
				}
				else
				{
            ...
            void OnGameObjectCreate(GameObject* go)
            {
                ...
                switch (go->GetEntry())
                {
					...
					//case GO_CACHE_OF_THE_DREAMWALKER_25H_1:
