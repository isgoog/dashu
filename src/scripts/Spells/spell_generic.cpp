            ...
            void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
            {
                ...
                switch (GetSpellInfo()->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_GENERIC:
						heal = std::max(1.0f * float(GetCaster()->SpellBaseDamageBonusDone(GetSpellInfo()->GetSchoolMask())), 1.0f * float(GetCaster()->GetTotalAttackPowerValue(BASE_ATTACK)));
						break;//wqcore更改德兰妮种族技能
                    default:
                        break;
                }
