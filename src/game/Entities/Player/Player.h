...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
		...
		TeamId GetTeam() const { return GetBattlegroundId() ? m_bgData.bgTeamId : m_team; }
		uint32 GetCFSTeam() const { return m_team; }
		//wqcore
		void LoadxxPlayerInfo();
		uint32 GetPlayerSkillCount();
		void ModifySkillCount(int32 bw);
		uint32 GetFuyin();
		uint32 GetTTcishu();
		uint32 GetTTjifen();
		uint32 GetTT2cishu();
		uint32 GetTTzhaohuan1();
		uint32 GetTTzhaohuan2();
		uint32 GetTT2jifen();
		uint32 GetTTboshu();
		uint32 GetTTsaodang();
		void ModifyTTcishu(int32 ttcs);
		void ModifyTTcishu2(int32 ttcs);
		void ModifyTTzhaohuan1(int32 ttcs);
		void ModifyTTzhaohuan2(int32 ttcs);
		uint32 GetTocid();
		uint32 GetIccid();
		uint32 GetKlzid();
		uint32 GetGleid();
		uint32 GetYaoshitoc();
		uint32 GetYaoshiicc();
		void ModifyJntp(int32 ttcs);
		void ModifyJnjx(int32 ttcs);
		void ModifyJnpy(int32 ttcs);
		void ModifyHuoli(int32 ttcs);
		void ModifyShangrenshuaxin(int32 ttcs);
		void Modifysxshangxian(int32 ttcs);
		uint32 GetHuoli();
		uint32 GetShangpin1();
		uint32 GetShangrenshuaxin();
		uint32 Getsxshangxian();
		uint32 GetShangpin2();
		uint32 GetShangpin3();
		uint32 GetShangpin4();
		uint32 GetShangpin5();
		...
		uint32 GetSp4();
		uint32 GetSp5();
		void SetHuoli(uint32 ll)
		{
			huolizhi = ll;
		}
		void Setsxshangxian(uint32 ll)
		{
			sxshangxian = ll;
		}
		void SetTocid(uint32 ll)
		{
			m_tocid = ll;
		}
		void SetKlzid(uint32 ll)
		{
			m_klzid = ll;
		}
		void SetGleid(uint32 ll)
		{
			m_gleid = ll;
		}
		void SetYaoshitoc(uint32 ll)
		{
			m_yaoshitoc = ll;
		}
		void SetYaoshiicc(uint32 ll)
		{
			m_yaoshiicc = ll;
		}
		void SetShangpin1(uint32 ll)
		{
			shangpin1 = ll;
		}
		void SetShangpin2(uint32 ll)
		{
			shangpin2 = ll;
		}
		void SetShangpin3(uint32 ll)
		{
			shangpin3 = ll;
		}
		void SetShangpin4(uint32 ll)
		{
			shangpin4 = ll;
		}
		void SetShangpin5(uint32 ll)
		{
			shangpin5 = ll;
		}
		void SetSp1(uint32 ll)
		{
			sp1 = ll;
		}
		void SetSp2(uint32 ll)
		{
			sp2 = ll;
		}
		void SetSp3(uint32 ll)
		{
			sp3 = ll;
		}
		void SetSp4(uint32 ll)
		{
			sp4 = ll;
		}
		void SetSp5(uint32 ll)
		{
			sp5 = ll;
		}
		void SetIccid(uint32 ll)
		{
			m_iccid = ll;
		}
		void SetTTcishu(uint32 ll)
		{
			TTcishu = ll;
		}
		void SetTTboshu(uint32 ll)
		{
			TTboshu = ll;
		}
		void SetTT2cishu(uint32 ll)
		{
			TTcishu2 = ll;
		}
		void SetTTzhaohuan1(uint32 ll)
		{
			TTzhaohuan1 = ll;
		}
		void SetTTzhaohuan2(uint32 ll)
		{
			TTzhaohuan2 = ll;
		}
		void SetTTsaodang(uint32 ll)
		{
			TTsaodang = ll;
		}
		uint32 Getcjjf()
		...
		void Modifycjjf(int32 ttcs);
		void Modifyzbcj(int32 ttcs);
		void Setzbcj(uint32 ll)
		{
			zbcj = ll;
		}
		void SetXC(uint32 ll)
		{
			xuanchuan = ll;
		}
		void SetXCYES(uint32 ll)
		{
			xuanchuanyes = ll;
		}
		void CantAuctionBuy();
		void ModifyTTjifen(int32 ttcs);
		void ModifyTTjifen2(int32 ttcs);
		void ModifyFuyin(int32 bw);
		void SaveCharacter(SQLTransaction& trans);
		uint32 GetPayJF();
		uint32 GetPayXC();
		uint32 GetJlJF();
		void SetPlayerVipLevel(uint32 vip)
		{
			Viplevel = vip;
		}
		uint32 GetPlayerViplevel();
		void RewardBattleCredit(uint32 bttype, bool iswiner);
		void SaveJF(); 
		uint32 GetJF();
		uint32 GetXC();
		uint32 GetXCYES();
		uint32 GetZZ();
		uint32 ModifyJF(int32 Value);
		uint32 ModifyZZ(int32 Value);
		uint32 m_action;
		uint32 m_sender;
		uint32 m_zaixian;
		uint32 m_huoyue;
		...
		void SetGetVipCredit(bool ll)
		{
			vipcreditget = ll;
		}
		bool LoginMSG(bool isFirstLogin = false);
		bool LoginS();
		bool LoginSt();
		void ModifyPlayerJfypoint(int32 jjpoint, uint8 lx)
		{
			uint32 depo;
			depo = fypoint[lx] + jjpoint;
			fypoint[lx] = (depo < 0 || depo >1000000000) ? 0 : depo;
		}

		
		void SetPlayerPvprankLevel(uint32 rank)
		{
			pvprank = rank;
		}
		uint32 GetPlayerPvprankLevel()
		{
			return pvprank;
		}
		void UpgradeSetFromItem(Item *item)
		{
			FromItem = item;
		}

		void UpgradeSetTargetItem(Item * item)
		{
			TargetItem = item;
		}
		Item* UpgradeGetTargetItem()
		{
			return TargetItem;
		}
		Item* UpgradeGetFromItem()
		{
			return FromItem;
		}
		...
		uint32 m_repeatIT1;
		bool DoSpamCheck(std::string message, uint32 a);
...
//        Spell* m_spellModTakingSpell;  // Spell for which charges are dropped in spell::finish
    ...
    private:
		...
		/* WQCORE PLAY数据定义区开始*/
		int32 JF,ZZ;
		uint32 SkillCount, xuanchuan, xuanchuanyes;
		Item* FromItem, *TargetItem;
		uint32 Viplevel, pvprank, Fuyin, TTcishu, TTjifen, TTboshu, TTsaodang, TTcishu2, TTjifen2, huolizhi, TTzhaohuan1, TTzhaohuan2;
		uint32 m_AddTalent, m_tocid, m_iccid, m_pvpadd, m_klzid, m_yaoshitoc, m_yaoshiicc, m_gleid;
		uint32 shangpin1, shangpin2, shangpin3, shangpin4, shangpin5, sp1, sp2, sp3, sp4, sp5, shangrenshuaxin, cjjf, zbcj;
		uint32 sxshangxian;
		bool vipcreditget;
