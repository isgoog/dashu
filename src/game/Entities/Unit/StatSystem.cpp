...
{ 
	...
	value = value + float(GetPlayerJfypoint(3) * 100);
	uint32 beilv = GetPlayerJfypoint(4);
	value = value * float(1.0f + (float(beilv) / 100.0f));//wqcore羽翼加成全属性
...
{ 
    ...
    //value += GetStat(STAT_AGILITY) * 2.0f;                  // armor bonus from stats//wqcore关闭敏捷加成护甲
    value += GetModifierValue(unitMod, TOTAL_VALUE);
	...
	value = value + (GetPlayerJfypoint(3) * 100);//羽翼加成护甲值
...
{ 
    ...
    value *= GetModifierValue(unitMod, TOTAL_PCT);
	uint32 xl = GetPlayerJfypoint(2);
	float xll = float(xl) / 100.0f;
	if (sWorld->getBoolConfig(SHENJIPEIZHI_ON))
	{
		uint32 tp = GetJntp() * 2000;
		uint32 jx = GetJnjx() * 4000;
		uint32 py = GetJnpy() * 6000;
		value = value + float(tp) + float(jx) + float(py);
	}
	else
	{
		uint32 tp = GetJntp() * 200;
		uint32 jx = GetJnjx() * 400;
		uint32 py = GetJnpy() * 600;
		value = value + float(tp) + float(jx) + float(py);
	}
	float pvp = float(m_pvpadd) / 100.0f;
	value = value * (1.0f + xll);
    sScriptMgr->OnAfterUpdateMaxHealth(this, value);
    SetMaxHealth((uint32)value);//wqcore 加成血量？？
}
...
{ 
	...
	uint32 gq = GetPlayerJfypoint(0);
	float gqq = float(gq) / 100.0f;
	if (sWorld->getBoolConfig(SHENJIPEIZHI_ON))
	{
		uint32 tp = GetJntp() * 1000;
		uint32 jx = GetJnjx() * 2000;
		uint32 py = GetJnpy() * 3000;
		val2 = val2 + float(tp) + float(jx) + float(py);
	}
	else
	{
	...
	val2 = val2 * (1.0f + gqq);
    SetModifierValue(unitMod, BASE_VALUE, val2);//wqcore攻击强度加成 未知需测试
...
{ 
	...
	//float a = float(sWorld->getIntConfig(BAOJI_BAOJI)) / 100.00f;
...
{ 
   ...
   // value += std::max<float>(GetStat(STAT_AGILITY) - GetCreateStat(STAT_AGILITY), 0.0f) * 2.0f;//wqcore关闭敏捷加成护甲
