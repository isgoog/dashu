...
{ 
	...
	uint32 fq = GetPlayerJfypoint(1);
	float fqq = float(fq) / 100.0f;
	if (sWorld->getBoolConfig(SHENJIPEIZHI_ON))
	{
		uint32 tp = GetJntp() * 1000;
		uint32 jx = GetJnjx() * 2000;
		uint32 py = GetJnpy() * 3000;
		DoneAdvertisedBenefit = DoneAdvertisedBenefit + float(tp) + float(jx) + float(py);
	}
	else
	{
		uint32 tp = GetJntp() * 100;
		uint32 jx = GetJnjx() * 200;
		uint32 py = GetJnpy() * 300;
		DoneAdvertisedBenefit = DoneAdvertisedBenefit + float(tp) + float(jx) + float(py);
	}
	DoneAdvertisedBenefit = DoneAdvertisedBenefit * (1.0f + fqq);
    return DoneAdvertisedBenefit;//wqcore法术伤害加成未知需要测试
}
...
{ 
	if (GetTypeId() == TYPEID_PLAYER)
		 ToPlayer()->ignore_next_check = true;
    if (mount)
...
void CharmInfo::InitCharmCreateSpells1()
{
...
void Unit::Kill(Unit* killer, Unit* victim, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const *spellProto)
{
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        {
			//获得怪物击杀奖励
			sObjectMgr->GetKillCretureCredit(player, creature);

            Loot* loot = &creature->loot;
            loot->clear();
			if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 1 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 1))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 2 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 2))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 3 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 3))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 4 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 4))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 5 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 5))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 6 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 6))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 7 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 7))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 8 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 8))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 9 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 9))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else if (sObjectMgr->isBossCoreA(creature->GetMap()->GetInstanceId()) == 10 && creature->GetCreatureTemplate()->rank >= 3)
			{
				if (uint32 lootid = sObjectMgr->GetRaid(creature, 10))//wqcore怪物死亡获取掉落
					loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
			}
			else
			{
				if (sObjectMgr->GetRaid(creature, 1) > 0 && creature->GetCreatureTemplate()->rank >= 3)
				{
					if (uint32 lootid = sObjectMgr->GetRaid(creature, 1))//wqcore怪物死亡获取掉落
						loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
				}
				...
				{
					if (uint32 lootid = creature->GetCreatureTemplate()->lootid)//wqcore怪物死亡获取掉落
						loot->FillLoot(lootid, LootTemplates_Creature, looter, false, false, creature->GetLootMode());
				}
			}
...
bool Unit::SetCharmedBy1(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
	if (!charmer)
		return false;
...
{ 
	...
	if (clicker->GetTypeId() == TYPEID_PLAYER)
		 clicker->ToPlayer()->ignore_next_check = true;
    return result;
}
