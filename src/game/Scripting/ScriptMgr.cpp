...
//wqcore
bool ScriptMgr::OnGossipSelectCode1(Player* player, Item* item, uint32 sender, uint32 action, const char* code)
{
	ASSERT(player);
	ASSERT(item);

	GET_SCRIPT_RET(ItemScript, item->GetScriptId(), tmpscript, false);
	return tmpscript->OnGossipSelect1(player, item, sender, action);
}
