...
//    friend class ACE_Singleton<AnticheatMgr, ACE_Null_Mutex>;
    ...
    static AnticheatMgr* instance()
       ...
       // void AnticheatDeleteCommand(uint32 guid);
		...
		void TeleportBack(Player *player, uint32 &key);
    private:
		bool SpeedHackDetection(Player* player, MovementInfo movementInfo, uint32 opcode);
		void FlyHackDetection(Player* player, MovementInfo movementInfo, uint32 opcode);
