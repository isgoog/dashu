...
void GameEventMgr::LoadFromDB()
{
	...
	sLog->outString("Loading Game Event BgBattleground Data...");
	{
		uint32 oldMSTime = getMSTime();

		//                                                   0         1
		QueryResult result = WorldDatabase.Query("SELECT eventEntry FROM game_event_battleground");

		if (!result)
			sLog->outString(">> Loaded 0 battleground  in game events. DB table `game_event_battleground` is empty.");
		else
		{
			uint32 count = 0;
			do
			{
				...
				if (event_id >= mGameEvent.size())
				{
					sLog->outErrorDb("`game_event_battleground`: game event id (%u) is out of range compared to max event id in `game_event`.", event_id);
					continue;
				}
			...
			sLog->outString(">> Loaded %u battleground in game events in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
		}
	}
...
void GameEventMgr::UnApplyEvent(uint16 event_id)
{
	...
	UpdateBattlegroundSettingsBg(event_id, false);
}
...
void GameEventMgr::ApplyNewEvent(uint16 event_id)
{
    ...
    // update bg holiday
    UpdateBattlegroundSettings();
	UpdateBattlegroundSettingsBg(event_id, true);
}
...
void GameEventMgr::UpdateBattlegroundSettingsBg(uint16 event_id, bool activate)
{
	...
	for (ActiveEvents::const_iterator itr = m_ActiveEvents.begin(); itr != m_ActiveEvents.end(); ++itr)
	{
		...
		case 100://双倍战场
			if (activate)
			{
				sWorld->START_BONUS_BG = true;
				//sWorld->SendWorldWideScreenText(70002);
			}
			else
			{
				sWorld->START_BONUS_BG = false;
			}
			break;
		case 101://用于战歌开始
			if (activate)
			{
				...
				{
					sWorld->START_BONUS_BG_KAISHI = true;
					sWorld->ResetSendBoxText();
				}

			}
			else
			{
				sWorld->START_BONUS_BG_KAISHI = false;
			}
			break;
		case 102://用于恢复战歌为官方战场
			if (activate)
			{
				sWorld->START_BONUS_BG_I = true;
				sWorld->START_BONUS_BG_POINT = 10;
				sWorld->START_BONUS_BG_POINT_KILL = 1;
				sWorld->ResetSendBoxText();
			}
			else
			{
				sWorld->START_BONUS_BG_I = false;
				sWorld->START_BONUS_BG_POINT = 50;
				sWorld->START_BONUS_BG_POINT_KILL = 0;
			}
			break;
		case 103://用于重置每日礼包
			if (activate)
		...
		case 104://用于阿拉希开启
			if (activate)
			{
				if (!sWorld->START_BONUS_BG_KAISHI_AS)
				{
					sWorld->START_BONUS_BG_KAISHI_ALX = true;
					sWorld->ResetSendBoxTextAlx();
				}

			}
			else
			{
				sWorld->START_BONUS_BG_KAISHI_ALX = false;
			}
			break;
		case 105://用于风暴开启
			if (activate)
			{
				if (!sWorld->START_BONUS_BG_KAISHI_AS)
				{
					sWorld->START_BONUS_BG_KAISHI_FB = true;
					sWorld->ResetSendBoxTextFb();
				}

			}
			else
			{
				sWorld->START_BONUS_BG_KAISHI_FB = false;
			}
			break;
		case 106://用于奥山
			if (activate)
			{
				sWorld->START_BONUS_BG_KAISHI_AS = true;
				sWorld->ResetSendBoxTextAs();
			}
			else
			{
				sWorld->START_BONUS_BG_KAISHI_AS = false;
			}
			break;
		case 111://用于活动1
			if (activate)
			{
				sWorld->ResetSendBoxTextHd1();
			}
			break;
		case 112://用于活动2
			if (activate)
			{
				sWorld->ResetSendBoxTextHd2();
			}
			break;
		case 113://用于活动3
			if (activate)
			{
				sWorld->ResetSendBoxTextHd3();
			}
			break;
		case 114://用于活动4
			if (activate)
			{
				sWorld->ResetSendBoxTextHd4();
			}
			break;
		case 115://用于活动5
			if (activate)
			{
				sWorld->ResetSendBoxTextHd5();
			}
			break;
		case 116://用于活动6
			if (activate)
			{
				sWorld->ResetSendBoxTextHd6();
			}
			break;
		case 117://用于活动6
			if (activate)
			{
				sWorld->ResetSendBoxTextHd7();
			}
			break;
		case 118://用于活动1
			if (activate)
			{
				sWorld->ResetSendBoxTextHd8();
			}
			break;
		case 119://用于发送活力值
			if (activate)
			...
			//	else
			//	{
			//		sWorld->SendWorldWideScreenText(70004);
			//	}
			break;
		case 120://用于活动1
			if (activate)
			{
				if (!sWorld->START_BONUS_WORLD)
				{
					sWorld->ResetSendBoxTextHd120();
				}
		...
		case 121://关闭活动
			if (activate)
			{
				sWorld->START_BONUS_WORLD = true;
				//sWorld->SendWorldWideScreenText(70002);
			}
			else
			{
				sWorld->START_BONUS_WORLD = false;
			}
			break;
		case 122://用于发放商人刷新次数
			if (activate)
			{
				sWorld->ResetMeirishuaxin();
			}
			break;
		case 123://用于活动1
			if (activate)
