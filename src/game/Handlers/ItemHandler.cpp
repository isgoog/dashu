    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i)
    {
        if (GemEnchants[i])
        {
			...
			if (Item* guidItem = _player->GetItemByGuid(gem_guids[i]))
			{
				...
				if (sWorld->getBoolConfig(BAOSHICHAIXIE_ON))//ÊÇ·ñ¿ªÆô±¦Ê¯²ðÐ¶wqcore
				{
					SpellItemEnchantmentEntry const* enchantEntry = sSpellItemEnchantmentStore.LookupEntry(OldEnchants[i]);
					SpellItemEnchantmentEntry const* enchantEntrynew = sSpellItemEnchantmentStore.LookupEntry(GemEnchants[i]);
					if (enchantEntry)
					{
						if (sWorld->getIntConfig(CORE_BAOSHI_ID) == enchantEntrynew->GemID)
						{
							_player->AddItem(enchantEntry->GemID, 1);
							_player->AddItem(enchantEntrynew->GemID, 1);
						}
					}
				}
			}
