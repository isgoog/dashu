...
void WorldSession::HandleAuctionSellItem(WorldPacket & recvData)
{
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        if (itemsCount == 1 && item->GetCount() == count[i])
        {
			...
			//sObjectMgr->xxAHRewardGg(_player, AH->owner, AH->item_template, AH->itemCount, AH->buyout);
        ...
        else // Required stack size of auction does not match to current item stack size, clone item and set correct stack size
        {
			...
			sObjectMgr->xxAHRewardGg(_player, AH->owner, AH->item_template, AH->itemCount, AH->buyout);//wqcore
            SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
void WorldSession::HandleAuctionPlaceBid(WorldPacket & recvData)
{
    ...
    if (price < auction->buyout || auction->buyout == 0)
	{
		...
		{
			player->CantAuctionBuy();
			SendAuctionCommandResult(auction->Id, AUCTION_PLACE_BID, ERR_AUCTION_ITEM_NOT_FOUND, 0);
			return;
		}
        if (auction->bidder > 0)
        {
    ...
    else
    {
		...
		else
		{
			...
			if (!sObjectMgr->xxAHRewardOwner(player, auction->owner, int32(auction->buyout)))
				return;
		}
        auction->bidder = player->GetGUIDLow();
