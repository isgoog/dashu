...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket & recv_data)
{
	...
	{
		if (_player->m_action >0)
		{
			switch (_player->m_action)
			{
			case 1:
				sObjectMgr->AddToBG3(_player);
				_player->m_action = 0;
				break;
			case 2:
				sObjectMgr->AddToBG(_player);
				_player->m_action = 0;
				break;
			case 3:
				sObjectMgr->AddToBG1(_player);
				_player->m_action = 0;
				break;
			case 7:
				sObjectMgr->AddToBG2(_player);
				_player->m_action = 0;
				break;
			case 101:
				sObjectMgr->AddToBG101(_player);
				_player->m_action = 0;
				break;
			case 111:
				sObjectMgr->AddToBG111(_player);
				_player->m_action = 0;
				break;
			case 112:
				sObjectMgr->AddToBG112(_player);
				_player->m_action = 0;
				break;
			case 113:
				sObjectMgr->AddToBG113(_player);
				_player->m_action = 0;
				break;
			case 114:
				sObjectMgr->AddToBG114(_player);
				_player->m_action = 0;
				break;
			case 115:
				sObjectMgr->AddToBG115(_player);
				_player->m_action = 0;
				break;
			case 116:
				sObjectMgr->AddToBG116(_player);
				_player->m_action = 0;
				break;
			case 117:
				sObjectMgr->AddToBG117(_player);
				_player->m_action = 0;
				break;
			case 118:
				sObjectMgr->AddToBG118(_player);
				_player->m_action = 0;
				break;
			case 120:
				sObjectMgr->AddToBG120(_player);
				_player->m_action = 0;
				break;
			case 123:
				sObjectMgr->AddToBG123(_player);
				_player->m_action = 0;
				break;
			case 124:
				sObjectMgr->AddToBG123(_player);
				...
				break;
			case 4470:
				sObjectMgr->AddToFuben3(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 4470);
				break;
			case 4535:
				sObjectMgr->AddToFuben4(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 4535);
				break;
			case 4131:
				sObjectMgr->AddToFuben3(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 4131);
				break;
			case 4135://上层
				sObjectMgr->AddToFuben3(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 4135);
				break;
			case 5191:
				sObjectMgr->AddToFuben(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5191);
				_player->m_action = 0;	
				break;
			case 5192:
				sObjectMgr->AddToFuben(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5192);
				_player->m_action = 0;
				break;
			case 5193:
				sObjectMgr->AddToFuben(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5193);
				_player->m_action = 0;
				break;
			case 5194:
				sObjectMgr->AddToFuben(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5194);
				_player->m_action = 0;
				break;
			case 5379:
				sObjectMgr->AddToFuben(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5379);
				_player->m_action = 0;
				break;
			case 5503:
				sObjectMgr->AddToFuben1(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5503);
				_player->m_action = 0;
				break;
			case 5505:
				sObjectMgr->AddToFuben(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5505);
				_player->m_action = 0;
				break;
			case 5670:
				sObjectMgr->AddToFuben2(_player, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), 5670);
				_player->m_action = 0;
				break;
			}
		}
	...
	if (IS_ITEM_GUID(guid))
	{
		...
		if (item)
			sScriptMgr->OnGossipSelectCode1(_player, item, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), _player->PlayerTalkClass->GetGossipOptionAction(gossipListId), code.c_str());
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
	...
	MapEntry const* entry = sMapStore.LookupEntry(at->target_mapId);
	uint32 targetDifficulty;
	targetDifficulty = player->GetDifficulty(entry->IsRaid());
	//sLog->outString("fubenjinrushinandu  (%u)", targetDifficulty);
	if (player->IsAlive())
	{
		switch (triggerId)
		{
		case 4747:
			...
			{
				ChatHandler(player).PSendSysMessage("|CFFFF0000只能进入英雄模式|r");
				teleported = true;
			}
			break;
		case 4983:
		...
		case 5635:
			...
			break;
		case 5505:
			sObjectMgr->SendSuoyaoText(player, triggerId);
			teleported = true;
			break;
		/*case 5503:
			if (targetDifficulty < 3)
			{
				ChatHandler(player).PSendSysMessage("|CFFFF0000只能进入25人英雄模式|r");
				teleported = true;
			}
			break;
		case 5670:
