...
World::World()
{
	START_BONUS_BG_POINT = 10;
	START_BONUS_BG_POINT_KILL = 1;
	START_BONUS_BG_POINT_I = 35;
	START_BONUS_BG_POINT_KILL_I = 0;
	START_BONUS_BG_I = false;
	START_BONUS_BG = false;
	START_BONUS_WORLD = false;
	START_BONUS_BG_KAISHI = false;
	START_BONUS_BG_KAISHI_ALX = false;
	START_BONUS_BG_KAISHI_FB = false;
	START_BONUS_BG_KAISHI_AS = false;
    m_playerLimit = 0;
...
void World::LoadConfigSettings(bool reload)
{
	...
	//wqcore
	m_int_configs[CONFIG_BATTLEGROUND_DAMAGE_LIMIT] = sConfigMgr->GetIntDefault("Battleground.Damage.Limit", 100000);
	m_int_configs[CONFIG_BATTLEGROUND_TREAT_LIMIT] = sConfigMgr->GetIntDefault("Battleground.Treat.Limit", 100000);
	m_int_configs[LOGIN_CHANGE_FACTION_JF] = sConfigMgr->GetIntDefault("Login.Change.Faction.Jf", 50000);
	m_int_configs[LOGIN_CHANGE_RACE_JF] = sConfigMgr->GetIntDefault("Login.Charge.Race.Jf", 50000);
	m_int_configs[LOGIN_CUSTOMIZE_JF] = sConfigMgr->GetIntDefault("Login.Customize.Jf", 50000);
	m_int_configs[LOGIN_RENAME_JF] = sConfigMgr->GetIntDefault("Login.Rename.Jf", 50000);
	m_int_configs[ZY_XUEXI_JF] = sConfigMgr->GetIntDefault("Zy.Xuexi.Jf", 50000);
	m_int_configs[ZY_TISHENG_JF] = sConfigMgr->GetIntDefault("Zy.Tisheng.Jf", 50000);
	m_int_configs[CORE_JF_ID] = sConfigMgr->GetIntDefault("Core.Jf.Id", 15);
	m_int_configs[ZY_HUANHUA_ITEM_ID] = sConfigMgr->GetIntDefault("Zy.Huanhua.Item.Id", 15);
	m_int_configs[ZY_HUANHUA_JF] = sConfigMgr->GetIntDefault("Zy.Huanhua.Jf", 1000);
	m_int_configs[CONFIG_AHSL_JF_ID] = sConfigMgr->GetIntDefault("Core.Ahsl.Jf.Id", 10);
	m_int_configs[CORE_GOLD_ID] = sConfigMgr->GetIntDefault("Core.Gold.Id", 0);
	m_int_configs[CORE_BAOSHI_ID] = sConfigMgr->GetIntDefault("Core.Baoshi.Id", 28);
	m_int_configs[CORE_DYH_ID] = sConfigMgr->GetIntDefault("Core.Dyh.Id", 100);
	m_int_configs[CORE_ASITEM_ID] = sConfigMgr->GetIntDefault("Core.Asitem.Id", 24);
	m_int_configs[CORE_ASITEM_ID_COUNT] = sConfigMgr->GetIntDefault("Core.Asitem.Id.Count", 1);
	m_int_configs[CONFIG_AUTOGIVERTIME] = sConfigMgr->GetIntDefault("Autogivertime.Time", 1);
	m_int_configs[CONFIG_AUTOGIVERJF] = sConfigMgr->GetIntDefault("Autogivertime.Jf", 0);
	m_int_configs[CORE_AUTOGIVERITEM_ID] = sConfigMgr->GetIntDefault("Autogiveritem.Id", 10);
	m_int_configs[CORE_SUOYAOITEM_ID] = sConfigMgr->GetIntDefault("Suoyaoitem.Id", 6);
	m_int_configs[CORE_AUTOGIVERITEM_ID_COUNT] = sConfigMgr->GetIntDefault("Autogiveritem.Id.Count", 1);
	m_int_configs[CORE_AUTOGIVERHONOR_COUNT1] = sConfigMgr->GetIntDefault("AutogiverHonor.Count1", 1);
	m_int_configs[CORE_AUTOGIVERHONOR_COUNT2] = sConfigMgr->GetIntDefault("AutogiverHonor.Count2", 1);
	m_bool_configs[CONFIG_AHSL] = sConfigMgr->GetBoolDefault("Ahsl.Enable", true);
	m_bool_configs[CONFIG_AUTOGIVERON] = sConfigMgr->GetBoolDefault("Autogiveron.Enable", true);
	m_bool_configs[CONFIG_AUTOVIPON] = sConfigMgr->GetBoolDefault("Autovipon.Enable", true);
	m_bool_configs[CONFIG_VIPUPON] = sConfigMgr->GetBoolDefault("Vipupon.Enable", true);
	m_bool_configs[BG_AS_ON] = sConfigMgr->GetBoolDefault("Bg.as.on.Enable", true);
	m_int_configs[WORLDSAY_GOLD] = sConfigMgr->GetIntDefault("Worldsay.Gold", 0);
	m_bool_configs[WORLDSAY_ON] = sConfigMgr->GetBoolDefault("Worldsay.on", true);
	m_int_configs[MINJIE_MINJIE] = sConfigMgr->GetIntDefault("Core.minjie.xishu", 18);
	m_int_configs[MINJIE_QITA] = sConfigMgr->GetIntDefault("Core.minjie.qtxishu", 0);
	m_int_configs[MINJIE_MINJIE_MINJIE] = sConfigMgr->GetIntDefault("Core.minjie.minjie.minjie", 10000000);
	m_int_configs[BAOJI_BAOJI] = sConfigMgr->GetIntDefault("Core.baoji.baoji", 10000000);
	m_int_configs[RAID_NAXXBUFF] = sConfigMgr->GetIntDefault("Core.Raid.Naxxbuff", 0);
	m_int_configs[RAID_ULDBUFF] = sConfigMgr->GetIntDefault("Core.Raid.Uldbuff", 0);
	m_int_configs[RAID_TOCBUFF] = sConfigMgr->GetIntDefault("Core.Raid.Tocbuff", 0);
	m_int_configs[RAID_ICCBUFF] = sConfigMgr->GetIntDefault("Core.Raid.Iccbuff", 0);
	m_int_configs[SUOYAO_BUFF] = sConfigMgr->GetIntDefault("Core.Suoyao.Buff", 931000);
	m_int_configs[RAID_1] = sConfigMgr->GetIntDefault("Core.Raid.1", 2);
	m_int_configs[RAID_BUFF0] = sConfigMgr->GetIntDefault("Core.Raid.Buff0", 62276);
	m_int_configs[RAID_BUFF1] = sConfigMgr->GetIntDefault("Core.Raid.Buff1", 39869);
	m_int_configs[RAID_BUFF2] = sConfigMgr->GetIntDefault("Core.Raid.Buff2", 62276);
	m_int_configs[RAID_BUFF3] = sConfigMgr->GetIntDefault("Core.Raid.Buff3", 39869);
	m_int_configs[RAID_BUFF4] = sConfigMgr->GetIntDefault("Core.Raid.Buff4", 62276);
	m_int_configs[RAID_BUFF5] = sConfigMgr->GetIntDefault("Core.Raid.Buff5", 39869);
	m_float_configs[RAID_VAS1] = sConfigMgr->GetFloatDefault("Core.Raid.Vas1", 2.0f);
	m_float_configs[RAID_VAS2] = sConfigMgr->GetFloatDefault("Core.Raid.Vas2", 3.0f);
	m_float_configs[RAID_VAS3] = sConfigMgr->GetFloatDefault("Core.Raid.Vas3", 4.0f);
	m_float_configs[RAID_VAS4] = sConfigMgr->GetFloatDefault("Core.Raid.Vas4", 5.0f);
	m_float_configs[RAID_VAS5] = sConfigMgr->GetFloatDefault("Core.Raid.Vas5", 6.0f);
	m_float_configs[RAID_VAS6] = sConfigMgr->GetFloatDefault("Core.Raid.Vas6", 7.0f);
	m_float_configs[RAID_VAS7] = sConfigMgr->GetFloatDefault("Core.Raid.Vas7", 8.0f);
	m_float_configs[RAID_VAS8] = sConfigMgr->GetFloatDefault("Core.Raid.Vas8", 9.0f);
	m_float_configs[RAID_VAS9] = sConfigMgr->GetFloatDefault("Core.Raid.Vas9", 10.0f);
	m_float_configs[RAID_VAS10] = sConfigMgr->GetFloatDefault("Core.Raid.Vas10", 11.0f);
	m_int_configs[GUARDITEM] = sConfigMgr->GetIntDefault("Guard.item", 0);
	m_int_configs[PROITEM] = sConfigMgr->GetIntDefault("Pro.item", 20);
	m_int_configs[PROCOUNT] = sConfigMgr->GetIntDefault("Pro.count", 1);
	m_int_configs[ADDPRO] = sConfigMgr->GetIntDefault("Add.pro", 5);
	m_int_configs[PROITEM1] = sConfigMgr->GetIntDefault("Pro.item1", 21);
	m_int_configs[PROCOUNT1] = sConfigMgr->GetIntDefault("Pro.count1", 1);
	m_int_configs[ADDPRO1] = sConfigMgr->GetIntDefault("Add.pro1", 10);
	m_int_configs[PROITEM2] = sConfigMgr->GetIntDefault("Pro.item2", 22);
	m_int_configs[PROCOUNT2] = sConfigMgr->GetIntDefault("Pro.count2", 1);
	m_int_configs[ADDPRO2] = sConfigMgr->GetIntDefault("Add.pro2", 15);
	m_int_configs[PROITEM3] = sConfigMgr->GetIntDefault("Pro.item3", 23);
	m_int_configs[PROCOUNT3] = sConfigMgr->GetIntDefault("Pro.count3", 1);
	m_int_configs[ADDPRO3] = sConfigMgr->GetIntDefault("Add.pro3", 20);
	m_int_configs[FUHUOITEM] = sConfigMgr->GetIntDefault("Fuhuo.item", 19);
	m_bool_configs[DYHON] = sConfigMgr->GetBoolDefault("Dyhon", true);
	m_bool_configs[BATTLEGROUNDS_ON] = sConfigMgr->GetBoolDefault("Battlegrouds.on", true);
	m_bool_configs[JIAREN_ON] = sConfigMgr->GetBoolDefault("Jiaren.on", true);
	m_bool_configs[SHENJIPEIZHI_ON] = sConfigMgr->GetBoolDefault("Shenjipeizhi.on", true);
	m_bool_configs[BATTLEGROUNDS_FANGSHUA_ON] = sConfigMgr->GetBoolDefault("Battlegrouds.Fangshua.on", false);
	m_bool_configs[BAOSHICHAIXIE_ON] = sConfigMgr->GetBoolDefault("BaoshiChaixie.on", true);
    // Warden
...
void World::SetInitialWorldSettings()
{
    ...
    mgr->LoadChannels();
	sObjectMgr->LoadAllxxCoreData();// wqcore自定义表加载
...
void World::Update(uint32 diff)
{
	...
	//wqcore泡点时间
	if (getBoolConfig(CONFIG_AUTOGIVERON))
	{
		if (m_gameTime > m_NextGiverTime)
			//发积分并重置泡点时间
			ResetGiverTime();
	}
...
void World::ResetSendBoxText()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGive1();

}

void World::ResetSendBoxTextAlx()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGive2();

}

void World::ResetSendBoxTextFb()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGive3();

}

void World::ResetSendBoxTextAs()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGive4();

}

void World::ResetSendBoxTextDyh()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGive101();

}

void World::ResetSendBoxTextHd1()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd1();

}

void World::ResetSendBoxTextHd2()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd2();

}

void World::ResetSendBoxTextHd3()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd3();

}

void World::ResetSendBoxTextHd4()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd4();

}

void World::ResetSendBoxTextHd5()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd5();

}

void World::ResetSendBoxTextHd6()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd6();

}

void World::ResetSendBoxTextHd7()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd7();

}

void World::ResetSendBoxTextHd8()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd8();

}

void World::ResetSendBoxTextHd120()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd120();

}

void World::ResetSendBoxTextHd123()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd123();

}

void World::ResetSendBoxTextHd124()
{

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->getLevel() == 80)
			itr->second->GetPlayer()->AutoGiveHd124();

}

//wqcore泡点系统
void World::ResetGiverTime()
{
	time_t curTime = time(NULL);

	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer())
			itr->second->GetPlayer()->AutoGive();
	//sLog->outString("m_NextGiverTime1  (%u)", m_NextGiverTime);
	//sObjectMgr->AddPlayerBot();
	//sLog->outString("TIMER:%u--JF:%u--GOLD:%u--YINZI:%u",sObjectMgr->GetIntConfig(AUTOGIVETIMER),sObjectMgr->GetIntConfig(AUTOGIVEGOLD),sObjectMgr->GetIntConfig(AUTOGIVEJF),sObjectMgr->GetIntConfig(AUTOGIVEYINZI));
	//m_NextGiverTime = m_NextGiverTime < curTime ? time_t(curTime + MINUTE) : m_NextGiverTime;
	m_NextGiverTime = m_NextGiverTime < curTime ? curTime : m_NextGiverTime;
	//m_NextGiverTime = time_t(m_NextGiverTime + MINUTE * 10);
	//sLog->outString("curTime  (%u)", curTime);
	//sLog->outString("m_gameTime  (%u)", m_gameTime);
	//sLog->outString("m_NextGiverTime2  (%u)", m_NextGiverTime);
	m_NextGiverTime = time_t(m_NextGiverTime + MINUTE * getIntConfig(CONFIG_AUTOGIVERTIME));
	//sLog->outString("m_NextGiverTime3  (%u)", m_NextGiverTime);
}


void World::ResetMeiri()
{

	LoginDatabase.PExecute("update xx_account_extra set vipcreditget=0");
	LoginDatabase.PExecute("update xx_account_extra set xuanchuanyes=0");
	CharacterDatabase.PExecute("update xx_character_extra set ttcishu=0");
	CharacterDatabase.PExecute("update xx_character_extra set ttsaodang=0");
	CharacterDatabase.PExecute("update xx_character_extra set ttcishu2=0");
	CharacterDatabase.PExecute("update xx_character_extra set ttzhaohuan1=0");
	CharacterDatabase.PExecute("update xx_character_extra set ttzhaohuan2=0");
	CharacterDatabase.PExecute("update xx_character_extra set sxshangxian=0");
	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer())
		{

			itr->second->GetPlayer()->SetGetVipCredit(false);
			itr->second->GetPlayer()->SetTTcishu(0);
			itr->second->GetPlayer()->SetTTsaodang(0);
			itr->second->GetPlayer()->SetTT2cishu(0);
			itr->second->GetPlayer()->SetTTzhaohuan1(0);
			itr->second->GetPlayer()->SetTTzhaohuan2(0);
			itr->second->GetPlayer()->Setsxshangxian(0);
			itr->second->GetPlayer()->SetXCYES(0);
		}
	sObjectMgr->setisBossCoreA(0);
}

void World::ResetMeirishuaxin()
{

	CharacterDatabase.PExecute("update xx_character_extra set shangrenshuaxin=shangrenshuaxin+1 where shangrenshuaxin < 10 ");
	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer())
		{
			if (itr->second->GetPlayer()->GetShangrenshuaxin() < 10)
			    itr->second->GetPlayer()->ModifyShangrenshuaxin(1);
		}
}

void World::ResetHuoli()
{

	CharacterDatabase.PExecute("update xx_character_extra set huolizhi=180");
	for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
		if (itr->second->GetPlayer())
		{
			itr->second->GetPlayer()->SetHuoli(180);
		}
}