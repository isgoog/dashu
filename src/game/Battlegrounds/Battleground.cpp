...
inline void Battleground::_CheckSafePositions(uint32 diff)
{
    ...
    if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL)
    {
        ...
        for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        {
            ...
            GetTeamStartLoc(itr->second->GetTeam(), x, y, z, o);
    ...
    else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
    {
        ...
        if (isArena())
        {
            ...
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                ...
                {
                    WorldPacket status;
                    sBattlegroundMgr->BuildBattlegroundStatusPacket(&status, this, player->GetCurrentBattlegroundQueueSlot(), STATUS_IN_PROGRESS, 0, GetStartTime(), GetArenaType(), player->GetTeam());
...
{
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        if (itr->second->GetTeam() == teamId && (self || sender != itr->second))
            itr->second->GetSession()->SendPacket(packet);
}
	...
	for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
		{
	      Player* player = itr->second;
			...
			if (itr2 == PlayerScores.end() || player->GetTeam() != teamId)
			 continue;
		
			BattlegroundScore* score = itr2->second;
		
			if (score->DamageDone >= sWorld->getIntConfig(CONFIG_BATTLEGROUND_DAMAGE_LIMIT) ||
			score->HealingDone >= sWorld->getIntConfig(CONFIG_BATTLEGROUND_TREAT_LIMIT)
...
void Battleground::RewardItemToTeam(uint32 item, uint32 itemcount, TeamId teamId)
{
	//sLog->outCommand(123, "RewardHonorToTeam honor %d teamId %d",
	//	honor, teamId);
	...
	for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
	{
		...
		if (itr2 == PlayerScores.end() || player->GetTeam() != teamId || item == 0)
			continue;
		...
		{
			itr->second->AddItem(item, itemcount);
		}
...
void Battleground::RewardReputationToTeam(uint32 a_faction_id, uint32 h_faction_id, uint32 reputation, TeamId teamId)
{
	...
	FactionEntry const* h_factionEntry = sFactionStore.LookupEntry(h_faction_id);
		...
		Player* plr = ObjectAccessor::FindPlayer(itr->first);
			...
			if (!plr)
			 {
						//sLog->outDebug("bg.battleground", "BattleGround:RewardReputationToTeam: %u not found!", itr->first);
				continue;
			}
		uint32 team = plr->GetTeam();
		
					//sLog->outCommand(123, "RewardReputationToTeam %s, player team : %d teamId: %d",
					//	plr->GetName().c_str(), plr->GetTeam(), teamId);
			
			if (team == teamId) {
			uint32 repGain = reputation;
			AddPct(repGain, itr->second->GetTotalAuraModifier(SPELL_AURA_MOD_REPUTATION_GAIN));
			AddPct(repGain, itr->second->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_FACTION_REPUTATION_GAIN, plr->GetCFSTeam() == TEAM_ALLIANCE ? a_faction_id : h_faction_id));
			itr->second->GetReputationMgr().ModifyReputation(plr->GetCFSTeam() == TEAM_ALLIANCE ? a_factionEntry : h_factionEntry, repGain);
			
				
		}
				//	plr->GetReputationMgr().ModifyReputation(plr->GetCFSTeam() == TEAM_ALLIANCE ? a_factionEntry : h_factionEntry, reputation);
			}
}
...
void Battleground::EndBattleground(TeamId winnerTeamId)
{
    ...
    {
        Player* player = itr->second;
        TeamId bgTeamId = player->GetTeam();
		...
		if (itr2 == PlayerScores.end() || player->GetTeam() != bgTeamId)
			continue;
		...
		damage = uint32(score->DamageDone / 10000);
		healing = uint32(score->HealingDone / 10000);
		killing = score->KillingBlows;
		deaths = score->Deaths;
		honorablekills = score->HonorableKills;
		damage = damage > 20 ? 20 : damage;
		healing = healing > 20 ? 20 : healing;
		killing = killing > 10 ? 10 : killing;
		deaths = deaths > 5 ? 5 : deaths;
		honorablekills = honorablekills > 5 ? 5 : honorablekills;
		uint32 huoyuedu = 0;
		huoyuedu = damage + healing + killing + deaths + honorablekills;
		if (sWorld->getBoolConfig(BATTLEGROUNDS_FANGSHUA_ON))//wqcore
		{
			if (huoyuedu > 25)
			{
				if (bgTeamId == winnerTeamId)
					player->RewardBattleCredit(uint32(this->GetBgTypeID()), true);
				else
					player->RewardBattleCredit(uint32(this->GetBgTypeID()), false);
			}
		}
		else
		{
        ...
        WorldPacket data;
        sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, this, player->GetCurrentBattlegroundQueueSlot(), STATUS_IN_PROGRESS, TIME_TO_AUTOREMOVE, GetStartTime(), GetArenaType(), player->GetTeam());
        player->GetSession()->SendPacket(&data);
...
void Battleground::UpdatePlayerScore(Player* player, uint32 type, uint32 value, bool doAddHonor)
{
	...
	if (sWorld->getBoolConfig(BATTLEGROUNDS_FANGSHUA_ON))
	{
		uint32 damage = 0;
		uint32 healing = 0;
		uint32 killing = 0;
		uint32 deaths = 0;
		uint32 honorablekills = 0;
		damage = uint32(itr->second->DamageDone / 10000);
		healing = uint32(itr->second->HealingDone / 10000);
		killing = itr->second->KillingBlows;
		deaths = itr->second->Deaths;
		honorablekills = itr->second->HonorableKills;
		damage = damage > 20 ? 20 : damage;
		healing = healing > 20 ? 20 : healing;
		killing = killing > 10 ? 10 : killing;
		deaths = deaths > 5 ? 5 : deaths;
		...
		uint32 huoyuedu = 0;
		huoyuedu = damage + healing + killing + deaths + honorablekills;
		ChatHandler(player).PSendSysMessage("|cffFF0000当前活跃度：%u", huoyuedu);//wqcore显示活跃度是否可行？
	}
...
void Battleground::HandleKillPlayer(Player* victim, Player* killer)
{
    ...
    if (killer)
    {
        ...
        for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        {
            ...
            if (creditedPlayer->GetTeam() == killer->GetTeam() && (creditedPlayer == killer || creditedPlayer->IsAtGroupRewardDistance(victim)))
...
void Battleground::PlayerAddedToBGCheckIfBGIsRunning(Player* player)
{
    ...
    sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, this, player->GetCurrentBattlegroundQueueSlot(), STATUS_IN_PROGRESS, GetEndTime(), GetStartTime(), GetArenaType(), player->GetTeam());
    player->GetSession()->SendPacket(&data);
}
...
uint32 Battleground::GetAlivePlayersCountByTeam(TeamId teamId) const
{
    uint32 count = 0;
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        if (itr->second->IsAlive() && !itr->second->HasByteFlag(UNIT_FIELD_BYTES_2, 3, FORM_SPIRITOFREDEMPTION) && itr->second->GetTeam() == teamId)
            ++count;
