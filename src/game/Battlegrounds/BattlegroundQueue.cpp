...
void BattlegroundQueue::FillPlayersToBG(const int32 aliFree, const int32 hordeFree, BattlegroundBracketId bracket_id)
{
    ...
    if (!sBattlegroundMgr->isTesting())
		...
		Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(this->m_bgTypeId);
	...
	if (sWorld->getBoolConfig(CROSSFACTION_SYSTEM_BATTLEGROUNDS))
		...
		m_SelectionPools[TEAM_HORDE].Init();
			...
			GroupsQueueType::const_iterator Horde_itr = m_QueuedGroups[bracket_id][BG_QUEUE_CROSSFACTION].begin();
		for (; Horde_itr != m_QueuedGroups[bracket_id][BG_QUEUE_CROSSFACTION].end(); ++Horde_itr)
			 {
			GroupQueueInfo* qinfo = (*Horde_itr);
			
				if (!qinfo->IsInvitedToBGInstanceGUID)
				 tmp.AddGroup(qinfo, hordeFree + aliFree);
			
				qinfo->seledted = 0;
			}
		
			
			int32 aliFreeCount = aliFree;
		int32 hordeFreeCount = hordeFree;
		
			
			while ((aliFreeCount != 0 || hordeFreeCount != 0) && tmp.GetWatingPlayerCount() > 0)
			 {	//	0				1
			if (aliFreeCount >= hordeFreeCount)
				 {
				if (aliFreeCount != 0)
					 {
					for (GroupsQueueType::const_iterator citr = tmp.SelectedGroups.begin(); citr != tmp.SelectedGroups.end(); ++citr)
						 {
						if (aliFreeCount < hordeFreeCount || aliFreeCount == 0)
							 break;
						
							GroupQueueInfo* qinfo = *(citr);
						
							if (qinfo->IsInvitedToBGInstanceGUID || qinfo->seledted)
							 continue;
						
							
							qinfo->seledted = 1;
						qinfo->teamId = TEAM_ALLIANCE;
						m_SelectionPools[TEAM_ALLIANCE].AddGroup(qinfo, aliFree);
						aliFreeCount--;
						}
					}
				}//        1					0
			else if (hordeFreeCount >= aliFreeCount)
				 {
				if (hordeFreeCount != 0)
					 {
					for (GroupsQueueType::const_iterator citr = tmp.SelectedGroups.begin(); citr != tmp.SelectedGroups.end(); ++citr)
						{
						if (hordeFreeCount < aliFreeCount || hordeFreeCount == 0)
							 break;
							...
							qinfo->seledted = 1;
						qinfo->teamId = TEAM_HORDE;
						m_SelectionPools[TEAM_HORDE].AddGroup(qinfo, hordeFree);
							...
							//sLog->outCommand(123, "aliFreeCount %d  hordeFreeCount %d tmp.GetPlayerCount() %d", 
								//aliFreeCount, hordeFreeCount, tmp.GetPlayerCount());
				}
...
bool BattlegroundQueue::FillXPlayersToBG(BattlegroundBracketId bracket_id, Battleground* bg, bool start)
 {
	...
	m_SelectionPools[TEAM_HORDE].Init();
		...
		if (sWorld->getBoolConfig(CROSSFACTION_SYSTEM_BATTLEGROUNDS))
		 {
		int32 aliFree = bg->GetMaxPlayersPerTeam();
		int32 hordeFree = bg->GetMaxPlayersPerTeam();
		SelectionPool tmp;
			...
			for (GroupsQueueType::const_iterator itr = m_QueuedGroups[bracket_id][BG_QUEUE_CROSSFACTION].begin(); itr != m_QueuedGroups[bracket_id][BG_QUEUE_CROSSFACTION].end(); ++itr)
			 {
			GroupQueueInfo* qinfo = *(itr);
			
				if (!qinfo->IsInvitedToBGInstanceGUID)
				 tmp.AddGroup(qinfo, aliFree + hordeFree);
