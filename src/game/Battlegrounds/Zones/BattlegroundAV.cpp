...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    if (entry == BG_AV_CreatureInfo[AV_NPC_A_BOSS])
    {
		...
		RewardReputationToTeam(729, 730, BG_AV_REP_BOSS, killer->GetTeam() == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_BOSS])
    {
		...
		RewardReputationToTeam(729, 730, BG_AV_REP_BOSS, TEAM_ALLIANCE);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN])
    {
		...
		RewardReputationToTeam(729, 730, BG_AV_REP_CAPTAIN, killer->GetTeam() == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
        RewardHonorToTeam(GetBonusHonorFromKill(BG_AV_KILL_CAPTAIN), TEAM_HORDE);
		RewardItemToTeam(sWorld->getIntConfig(CORE_ASITEM_ID), sWorld->getIntConfig(CORE_ASITEM_ID_COUNT), TEAM_HORDE);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN])
    {
		...
		RewardItemToTeam(sWorld->getIntConfig(CORE_ASITEM_ID), sWorld->getIntConfig(CORE_ASITEM_ID_COUNT), TEAM_ALLIANCE);
    ...
    switch (questid)
    {
        ...
        case AV_QUEST_H_COMMANDER1:
           ...
           // RewardReputationToTeam(teamId, 1, teamId);
			RewardReputationToTeam(teamId, oteam, 1, player->GetTeam());
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
		...
		RewardItemToTeam(sWorld->getIntConfig(CORE_ASITEM_ID), sWorld->getIntConfig(CORE_ASITEM_ID_COUNT), ownerId);
